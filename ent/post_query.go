...
func (pq *PostQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, post.UsersTable, post.UsersPrimaryKey...),
		)
...
func (pq *PostQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Post, error) {
	...
	if query := pq.withUsers; query != nil {
		if err := pq.loadUsers(ctx, query, nodes,
			func(n *Post) { n.Edges.Users = []*User{} },
			func(n *Post, e *User) { n.Edges.Users = append(n.Edges.Users, e) }); err != nil {
			return nil, err
		}
	}
...
func (pq *PostQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*Post, init func(*Post), assign func(*Post, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(post.UsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(post.UsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(post.UsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(post.UsersPrimaryKey[1]))
