...
func (r *postsRepository) Create(input request.PostRequest) (*ent.Post, error) {
	...
	res, err := r.db.Post.Create().SetTitle(input.Title).SetSlug(input.Slug).SetImg(input.Image).SetBody(input.Body).Save(r.context)
...
func (r *postsRepository) UpdateById(id int, input request.PostRequest) (*ent.Post, error) {
	...
	res, err := r.db.Post.UpdateOneID(id).SetTitle(input.Title).SetSlug(input.Slug).SetImg(input.Image).SetBody(input.Body).Save(r.context)
